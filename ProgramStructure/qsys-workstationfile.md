...
[The IBM i RPG Application development Model]({{ site.rooturl }}/ibmi-developer-model/) includes the concept of a [WORKSTN File](https://www.ibm.com/support/knowledgecenter/en/ssw_ibm_i_72/rzasc/hintapp.htm)
...
Just as we have discussed how **ASP.NET 5 Website Architecture** [simplifies complexity by separating data from presentation logic]({{ site.rooturl }}/asp-net-websites/), Monarch further architects the Interactive Application by separating the Business Logic from the `Display Page` Logic.
...
3. MonaLisa - Monarch Application Server. Coordinates which Session's Active Program should process the [DataSet]({{ site.rooturl }}/qsys-expo-display-pages/) requested.
...
3. The only common concept between the `yellow` and `blue` threads is the `green` [DataSet]({{ site.rooturl }}/qsys-expo-display-pages/).
...
3. Any data-logic (including *indicators*) needs to be placed on the *intersection* of the `Yellow` and `Blue`, that is, the `Green` [DataSet]({{ site.rooturl }}/qsys-expo-display-pages/).
