...
```cs
computeCharLength(Type tLen, ref Int32 length);
```
...
```cs
computeDecimalLength(Type tDigits, Type tDecimals, ref Int32 digits, ref Int32 decimals);
```
...
```cs
computeDim(Type tDim, ref Int32 dim);
```
...
```cs
GetDateTimeObject(ASNA.QSys.Runtime.DateTimeDataKind kind, ASNA.DataGate.Common.DateTimeFormat format, ASNA.QSys.Runtime.DateTimeSeparator sep);
```
...
```cs
parseDateTimeType(ASNA.QSys.Runtime.DateTimeDataKind kind, Type Tformat, Type Tseparator, ref ASNA.DataGate.Common.DateTimeFormat format, ref ASNA.QSys.Runtime.DateTimeSeparator separator);
```
